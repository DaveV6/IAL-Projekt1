...
int nextIndex( int index ) {
	return (index + 1) % QUEUE_SIZE; // return index of the next element
}
...
int Queue_IsEmpty( const Queue *queue ) {
	return queue->firstIndex == queue->freeIndex; // return 1 if the queue is empty, 0 otherwise
}
...
int Queue_IsFull( const Queue *queue ) {
	return nextIndex(queue->freeIndex) == queue->firstIndex; // return 1 if the queue is full, 0 otherwise
}
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	if(!(Queue_IsEmpty(queue))) {
		Queue_Front(queue, dataPtr); // get the first element from the queue
...
void Queue_Enqueue( Queue *queue, char data ) {
	if(!(Queue_IsFull(queue))) {
		queue->array[queue->freeIndex] = data; // if the queue is not full, set the free index to the data
